import { BS_HIGH, BS_LOW, DEFAULT_PREFERENCE_WIDGETS, WIDGET_KEYS, type WidgetKey } from "./constants.js";
...
export function normalizeStoredDashboardWidgets(value: unknown): WidgetKey[] {
...
export function normalizeDashboardWidgets(widgets: readonly WidgetKey[]): WidgetKey[] {
...
export type ProfileRow = { weight: number | null; height: number | null } | null;
...
export function buildWidget(key: WidgetKey, records: RecordRow[], profile: ProfileRow, range: string, now: Date): WidgetResult {
  switch (key) {
    case "summary":
...
export function getDatetimeFilter(range: "7d" | "30d" | "all", multiplier = 1, now = new Date()): { gte: Date } | undefined {
