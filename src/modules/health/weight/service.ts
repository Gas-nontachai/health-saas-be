...
import { deleteWeightEntryByDate, findWeightEntriesPage, upsertWeightGoal } from "./repository.js";
import type { EntryListQuery, GoalBody } from "./schemas.js";

export async function listWeightEntries(prisma: AppPrisma, userId: string, query: EntryListQuery) {
  const { entries, totalCount } = await findWeightEntriesPage(prisma, userId, query);
...
export async function deleteWeightEntry(prisma: AppPrisma, userId: string, date: Date) { const result = await deleteWeightEntryByDate(prisma, userId, date); if (result.count === 0) throw new HttpError(404, "Weight metric entry not found"); }
export async function saveWeightGoal(prisma: AppPrisma, userId: string, body: GoalBody) { return upsertWeightGoal(prisma, userId, body); }
import {
  buildForecastSeries,
